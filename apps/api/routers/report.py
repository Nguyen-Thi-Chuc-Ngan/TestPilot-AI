...
async def get_report(job_id: str, user: dict = Depends(get_current_user)):
    ...
    findings = get_sb().table("findings").select("*").eq("job_id", job_id).execute().data or []
    test_cases = get_sb().table("test_cases").select("*").eq("job_id", job_id).execute().data or []
    bug_reports = get_sb().table("bug_reports").select("*").eq("job_id", job_id).execute().data or []
    artifacts = get_sb().table("artifacts").select("*").eq("job_id", job_id).execute().data or []
